...
with tab_find:
    ...
    st.markdown("""
##### 🟢 1. 감정평가액은 안 움직인다 — 그리고 그건 '제도의 합리적 결과'다
공개기간 중 **83.6%가 감정가 무변동**. 왜? 재감정은 채권자가 **의뢰해야만** 일어나는데,
인하 재감정은 채권자에겐 충당금 인식의 공식화·평가사에겐 책임 부담 — 누구도 의뢰할
유인이 없다(대리인 이론). 조정은 의뢰가 필요 없는 **최저입찰가 인하**로 우회한다(월간
하향 재감정 1.46%). 따라서 **공시 감정가는 '현재 가치'가 아니라 '마지막 감정 시점의
가치'**로 읽어야 한다.

##### 🟢 2. 공매 가격표는 시장이 아니라 '규칙'이 쓴다 — 관리가격
회차당 저감의 **61%가 정확히 −10% 또는 −5%**(관행 저감률, 7,132쌍 실측), 시작가도
감정가의 100·120·130% 같은 이산 배수에 몰린다(34%). 즉 **120% 시작 → 회차당 −9.5%
→ 8회차 62%** 경로는 시장 가격발견이 아니라 **관리가격**이며, 시장 변수와 동행하지
않는 것도 정의상 귀결이다. **시장의 정보는 경로 모양이 아니라 '언제 멈추는가'(낙찰·
종료 시점)와 규칙 밖 재량 저감에 있다** — 기대치 캘리브레이션엔 그대로 유효.

##### 🟢 3. 산업시설(물류)의 부실은 입지 문제가 아니다 — 4중 독립 증거 + 빈티지
① 감정가/실거래 괴리 최소(1.76배) ② 소재지 공장창고 거래량 수축(YoY −8.9%)
③ 시장보다 싸게 내놓아도(초과할인 −4.4%p) 체류 최장(11개월) ④ **정상 리츠 물류센터와
입지 3지표 전부 통계적 무차이 — 카카오 내비 실주행 검증에서도 유지**. 원인 후보는
공급 타이밍: 부실 산업시설 **허가의 64%가 2020~2022 저금리 붐 빈티지**(타 유형 43%,
OR 2.4 p=0.008 — 거미집 가설 정합. 단 시장 전체 기준선이 없어 '과잉' 인과는 미식별).
""")
...
##### 🧭 6. 왜 그런가 — 경제학 이론 검증 (신규 가설 검정 결과)
기존 발견을 한국 제도·경제학 이론으로 감사하고 신규 가설 5종을 검정했다
(상세: docs/PHASE1_findings.md §9):

- **🟡 주거 +20%p 초과할인 = 손실 확정 회피(처분 효과)의 지문**: 프리미엄이 시작가에서
  오고(회차 무관), 준거점이 굳은 구코호트만 +25%p로 버티며, 신규 매물은 처음부터 시장
  근접. 산업(−4.4%p)이 자연 대조군 — 손실 인식을 미룰 이해관계자 구조가 없는 유형은
  시장 수준에서 출발한다. (행태 vs 충당금 유인 분리는 내부 데이터 필요)
- **🟡 부실 숙박 = 관광지 자산**: 40곳 중 서울 도심 호텔 0곳, 강원·제주·전남 43~45%,
  리조트·펜션류 74% — 리츠 운용 호텔(수도권 86%)과 정반대(p=0.015). 용도전환이 어려운
  자산특수성이 처분 경로를 막는다.
- **🟡 '규모의 계곡'은 없다 — 대신 100억 문턱**: 중간 규모가 가장 안 팔린다는 가설은
  기각(U자 없음). 실제 구조는 **낙찰이 100억 미만에서만 유의하게 발생**(14.3% vs 4.9%,
  p=0.016 — 개인 매수 가능 구간)하고 500억+ 대형은 명단에 가장 오래 남는 "문턱+정체".
  (낙찰 22건 소표본 — 방향 등급)
- **🟡 신탁사 재량이 자산 특성보다 크다**: 시장 대비 가격 스탠스(초과할인)는 관리
  신탁사 간 유의하게 다르고(p=0.0008, 중앙값 격차 ~23%p, 구성 착시 아님) 설명력이
  유형·권역·시도를 압도(ε² 0.114 vs ≤0.002). 단 저감 깊이·체류는 신탁사 간 무차이 —
  기관 재량은 '시장 대비 버티기' 한 채널에 국한. (선택 표본 30% 한계)
- **🔵 지방 열위 = 한계지·얇은 시장 이론과 3중 정합**: 가격 발견 부재(지방 감정배수
  4.59)·정상화 희소(2.6%)·정상 리츠조차 지방 하향 3.7배. '인구감소 때문'은 아직
  과주장 — 소멸위험지수 결합 검정이 다음 단계.

##### 🔵 7. 방향 신호들 (탐색 등급 — 인용 주의)
